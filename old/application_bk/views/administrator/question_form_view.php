...
<?php endif; ?>
    ...
    <fieldset>
        ...
        <script type="text/javascript">
            ...
            {
                if((document.getElementById('is_mandatory').checked==false) && document.getElementById('no_mandatory').checked==false)
                {
                    alert("Please check question is mandatory!!!!");
                    return false;
                }
        ...
        <div class="form-actions">
            ...
            <input type="button" onclick="validateForm()" value="<?php if($is_edit) echo 'Update'; else echo 'Add'; ?> Question" class="btn btn-primary btn-large" />&nbsp;&nbsp;
...
jQuery(document).ready(function(){
    ...
    jQuery('#add-choice-button').live('click', function(){
        ...
        var mcqChoiceText = '';
        if($("#mcqchoice-div").find('.choice').length==1)
        {
            var fid=$.trim($("#mcqchoice-div").find('.choice').children("textarea").val());
            if(fid.length<1)
            {
                $("#mcqchoice-div").html("");
            }
