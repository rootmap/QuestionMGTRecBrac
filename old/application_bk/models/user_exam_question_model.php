...
class User_exam_question_model extends CI_Model
{
    ...
    private $table_name_question_set = 'question_set';
    ...
    public function get_user_exam_questions($user_exam_id)
    {
        ...
        if ($user_exam_id > 0) {
            ...
            $this->db->limit(1);
            $querySet = $this->db->get($this->table_name);
            $rowSET=$querySet->row_array();
            $setID=$rowSET['qus_set'];

            $this->db->where('id', $setID);
            $this->db->limit(1);
            $querySetInfo = $this->db->get($this->table_name_question_set);
            $rowSETInfo=$querySetInfo->row_array();
            $setIDName=$rowSETInfo['random_qus'];
