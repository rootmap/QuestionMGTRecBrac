        ...
        if ($records) {
            ...
            for ($i = 0; $i<count($records); $i++) {
                ...
                if ($default_cat_id != $records[$i]->id) {
                    $action_str .= '&nbsp;&nbsp;&nbsp;';
                    if(!isSystemAuditor())
                    $action_str .= anchor('administrator/category/delete/'. $records[$i]->id, '<i class="icon-trash"></i>', array('title'=>'Delete', 'onclick'=>'return confirm(\'Do you really want to delete this record?\')'));
                }
