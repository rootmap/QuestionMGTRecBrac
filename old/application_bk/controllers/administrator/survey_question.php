    ...
    public function index()
    {
        ...
        if ($records) {
            ...
            $tbl_heading = array(
                ...
                '2' => array('data'=> 'Status'),
                '3' => array('data'=> 'Type', 'class' => 'center', 'width' => '120'),
                '4' => array('data'=> 'Is Expired?', 'class' => 'center', 'width' => '80'),
                '5' => array('data'=> 'Action', 'class' => 'center', 'width' => '50')
            );
            ...
            for ($i = 0; $i<count($records); $i++) {
                ...
                if($records[$i]->qus_status==1){
                    $statusQus = '<a href="'.base_url("appreject/$suid/2").'" class="label label-important">Reject</a>';
                }else if($records[$i]->qus_status==2){
                    $statusQus = '<a href="'.base_url("appreject/$suid/1").'" class="label label-success">Approve</a>';
                }else{
                ...
                $tbl_row = array(
                    ...
                    '3' => array('data'=> $question_type, 'class' => 'center', 'width' => '120'),
                    '4' => array('data'=> $expired_str, 'class' => 'center', 'width' => '80'),
                    '5' => array('data'=> $action_str, 'class' => 'center', 'width' => '50')
                );
            ...
            if ($file_error == '') {
                ...
                if (count($questions) > 0) {
                    foreach($questions as $row => $question) {
                        ...
                        if (($question_type != 'option_based') && ($question_type != 'mcq') && ($question_type != 'descriptive')) {
                            $question_type = '';
                            $error_messages[$row][] = 'Question Type should be \'option_based\' or \'descriptive\'';
                            $row_has_error = true;
                        }

                        if($question_type=="option_based" || $question_type=="mcq")
                        {
                            $question_type = "option_based";
                        }

                        //print_r_pre($row_has_error); die();
