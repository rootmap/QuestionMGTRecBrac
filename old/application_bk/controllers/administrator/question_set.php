      ...
      public function index()
      {
        ...
        if ($records) {
          ...
          for ($i = 0; $i<count($records); $i++) {
            ...
            $action_str .= anchor('administrator/question_set/questionSetPreview/'. $records[$i]->id, '<i class="icon-eye-open"></i> ', array('title'=>'Preview','class'=>'btn btn-success','target'=>'_blank'));
              $action_str .= '&nbsp;&nbsp;&nbsp;';
              if(!isSystemAuditor())
...
';
    ...
    public function questionSetPreview(){
        ...
        //var_dump($question_set_details->name);die;
        tcpdf();
      ...
      $strHtml .='<table border="0" width="100%">
        ...
        for($i=0; $i<count($questions); $i++){
            $quesNo=$i+1;
            $strHtml .=  $quesNo. ') ' .html_entity_decode($questions[$i]->ques_text) . '&nbsp;('.$questions[$i]->mark.')';
            $strHtml .='<br>';
