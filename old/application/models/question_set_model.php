...
class Question_set_model extends CI_Model
{
    ...
    public function get_question_by_ques_set_id($ques_set_id,$rand=0)
    {
    ...
    public function get_question_by_ques_cat_set_id($ques_set_id,$cat_id=0,$randID=0)
    {
        ...
        if ($ques_set_id > 0) {
            ...
            if($randID==1)
            {
                $sqlStrOrder=" ORDER BY NEWID()";
            }
            $sql = 'select ques_text,ques_choices,ques_type,mark from exm_questions where id in (select question_id from exm_question_set_question_map where question_set_id='.$ques_set_id.') AND category_id='.$cat_id.''.$sqlStrOrder;
           // echo $sql; die();
            $res = $this->db->query($sql);
