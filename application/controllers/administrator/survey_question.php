...
class Survey_question extends MY_Controller
{
    ...
    public function add_question()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $survey_weight = $this->input->post('survey_weight');
    ...
    public function edit()
    {
        ...
        $this->form_data->survey_weight = $question->survey_weight;
    ...
    public function update_question()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->form_data->survey_weight = $survey_weight;
        ...
        if ($_FILES['question_file']['tmp_name'] != '' && $_FILES['question_file']['error'] == 0) {
            ...
            if ($file_error == '') {
                ...
                for ($i=$start; $i<=count($sheetData); $i++) {
                    ...
                    $survey_weight = $sheetData[$i]['I'];
                    $question_expiry_date = trim($sheetData[$i]['J']);
                    ...
                    if ($question_category == '' && $question_text == '' && $question_type == '' && $question_choices == '' && $question_expiry_date == '') {
                        ...
                        $questions[$i]['survey_weight'] = $survey_weight;
                ...
                if (count($questions) > 0) {
                    foreach($questions as $row => $question) {
                        ...
                        $survey_weight = $question['survey_weight'];
                        ...
                        if ($row_has_error) {
                            ...
                            $questions[$row]['survey_weight'] = $survey_weight;
        ...
        if ($_FILES['question_file']['tmp_name'] != '' && $_FILES['question_file']['error'] == 0) {
            ...
            if ($file_error == '') {
                ...
                for ($i=$start; $i<=count($sheetData); $i++) {
                    ...
                    $survey_weight = trim($sheetData[$i]['G']);
                    $question_expiry_date = trim($sheetData[$i]['H']);
    ...
    private function _set_rules()
    {
        ...
        $this->form_validation->set_rules('survey_weight', 'Survey Weight', 'required|trim');
