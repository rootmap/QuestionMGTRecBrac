...
class Exam extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $all_survey_categories_tree = $this->category_model->get_survey_categories_recursive();
        $all_survey_categories = $this->category_model->get_survey_padded_categories($all_survey_categories_tree);
        $this->cat_survey_lists=$all_survey_categories;



        $this->cat_survey_lists[] = 'Select a Category';
        if ($all_survey_categories) {
            for ($i=0; $i<count($all_survey_categories); $i++) {
                $this->cat_survey_lists[$all_survey_categories[$i]->id] = $all_survey_categories[$i]->cat_name;
            }
        }
    ...
    public function index()
    {
        $update_data = $this->insert_global_model->globalinsert($this->tbl_exam_users_activity,array('user_id'=>$this->logged_in_user->id,
            'activity_time'=>date('Y-m-d H:i:s'),'activity'=>'Exam Started View'));
    ...
    private function update_user_answer($exam)
    {
        ...
        if ($is_mcq) {
            ...
            $score_per_question = number_format($exam->exam_score / $exam->exam_total_questions, 2);
            $ques_score = (float)$exam->exam_questions[$current_question_index]->question->mark;
            $total_choices = count($ques_choices);
            $dontknow_answered = 0;
            $no_of_correct_choices = 0;
            $no_of_user_correct_choices = 0;
            //print_r_pre($ques_score);
            for ($i=0; $i<$total_choices; $i++) {
              ...
              //  echo $exam->exam_questions[$current_question_index]->question->ques_choices[$i]['is_user_answer']."<br>";


            }
            $wng = 0;
            foreach ($exam->exam_questions[$current_question_index]->question->ques_choices as $keys => $values) {
                if($values['is_answer']!=$values['is_user_answer']){
                     $wng++;
                }
            }
             //   echo $wng;
            // print_r_pre($exam->exam_questions[$current_question_index]->question);
    ...
    private function prepare_result_data($user_exam_id, $exam)
    {
        ...
        if ($exam_is_mcq) {
             //echo "<pre>";
           /// print_r_pre($exam->exam_questions)."<br>";
            for ($i=0; $i<count($exam->exam_questions); $i++) {
              //  print_r($exam->exam_questions[$i]);
