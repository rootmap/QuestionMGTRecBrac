...
class Home extends MY_Controller
{
    ...
    public function download_admitcard()
    {
       ...
       // print_r_pre($exam_details);
        $dataQrcCodeString="";
        $dataQrcCodeString .="Exam ID: ".$exam_id;
        $dataQrcCodeString .=" \r\n Exam Name: ".$exam_details->exam_title;
        $dataQrcCodeString .=" \r\n Role ID: XXXXX";
        $dataQrcCodeString .=" \r\n Role Name: XXXXX";
        $dataQrcCodeString .=" \r\n Exam Date: ".$dateofExam;
        $dataQrcCodeString .=" \r\n Location: ".$venuesLocation;

        //echo $dataQrcCodeString; die();
        //§  Questionnaire Set No.: A

        //§  Questionnaire Preparation Date: DD-MM-YY

        //§  Questionnaire Approver ID: Approver’s User ID (Users must have a unique ID through which s/he will log in which will be used as Approver ID too)*/

        $UpQRFilePath=base_url().'/assets/qrcode/index.php?data='.urlencode($dataQrcCodeString);
        ...
        $tableCellStyleData=" 
              ...
              vertical-align: inherit; padding:4px !important; border-style: none !important;";
...
<div style=" text-align:center; border:1px #000 solid;">
   ...
   <tbody>
         ...
         <td style="font-size:10px;" width="100%">
            <table>
                <tbody >
                    ...
                    $dataLocation=explode(',',$venuesLocation);
            ...
            if(count($dataLocation)>4)
            {
                $html .= '<tr>
                    ...
                    <td  align="left">
                           <b font-size="5">: '.$dataLocation[0].', '.$dataLocation[1].', '.$dataLocation[2].', '.$dataLocation[3].'</b>
                    </td>
                    </tr>';
                ...
                $html .= '<tr>
                    ...
                    $dfi=4;
                    foreach ($dataLocation as $localadd) {
                        if(!empty($dataLocation[$dfi]))
                        {
                            if($dfi>4)
                            {
                                $html .=', '.$dataLocation[$dfi];
                            }
                            else
                            {
                                $html .=$dataLocation[$dfi];
      ...
      <td width="23%">
            ...
            <br><b>'.$signaturenAdmitHeadD->user_first_name.' '.$signaturenAdmitHeadD->user_last_name.'</b><br />
            <b>'.$signaturenAdmitHeadD->designation.'</b><br />
            <b>'.$signaturenAdmitHeadD->department.'</b><br /><b>BRAC Bank Limited</b></p>
   ...
   <ul style="font-size: 10px;font-family: "Franklin Gothic Medium", "Franklin Gothic", "ITC Franklin Gothic", Arial, sans-serif;">
      ...
      <li> Personal calculator, Mobile Phones or any other electronic devices are not allowed un the examination hall. Candidates should not bring any additional documents into the examination hall. BRAC Bank authority will not be responsible for safe keeping of candidates personal belongings.</li>
   </ul>
   </div>
