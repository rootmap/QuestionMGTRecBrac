...
class Survey_report_model extends CI_Model
{
    ...
    public function get_survey($limit, $offset = 0, $filter = array())
    {
        ...
        $this->db->select($this->table_name_surveys.".survey_title, 
            ...
            ".$this->table_name_surveys.".survey_description,
            (SELECT count(ta.id) FROM exm_".$this->table_name_survey_questions." as ta WHERE ta.survey_id=exm_".$this->table_name_surveys.".id) as total_question,  
            (SELECT ISNULL(sum(CAST(td.survey_weight as float)),0) as totalW FROM exm_".$this->table_name_survey_questions." as ta 
            LEFT JOIN exm_survey_questions as td ON td.id=ta.question_id 
            WHERE ta.survey_id=exm_".$this->table_name_surveys.".id) as total_weight, 
            (SELECT count(ta.id) FROM exm_surveys_users as ta WHERE ta.survey_id=exm_".$this->table_name_surveys.".id) as total_assign_user,
            (SELECT count(ta.id) FROM exm_surveys_users as ta WHERE ta.survey_id=exm_".$this->table_name_surveys.".id AND ta.status='completed') as survey_attendies");
        ...
        $this->db->order_by($this->table_name_surveys.'.id','DESC');
    ...
    public function get_survey_bk($limit, $offset = 0, $filter = array())
    {
        $result = array();
        $result['result'] = false;
        $result['count'] = 0;
        ...
        //$this->db->order_by($this->table_name_surveys.'.survey_title','ASC');
        //$this->db->order_by($this->table_name_questions.'.ques_text','ASC');
        $this->db->order_by($this->table_name_questions.'.id','DESC');
    ...
    public function get_survey_summary($limit, $offset = 0, $filter = array())
    {
        //print_r($filter); die();
        $result = array();
        $result['result'] = false;
        ...
        if (is_array($filter) && count($filter) > 0) {
            foreach($filter as $key => $value) {
                if($filter[$key]['field']=="survey_id_filter")
                {
                    $this->db->where($this->table_name_survey_questions.'.survey_id', $filter[$key]['value']);
                }
                else
                {
                    $this->db->where($this->table_name_answer.'.'.$filter[$key]['field'], $filter[$key]['value']);
